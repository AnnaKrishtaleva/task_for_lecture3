...
void SerialGaussMethod(double **matrix, const int rows, double* result)
{
	...
	printf("Forward elimination duration = %f ticks\n\n", duration.count());
...
/// Функция SerialGaussMethod() решает СЛАУ методом Гаусса, с использованием Intel Cilk Plus
/// matrix - исходная матрица коэффиициентов уравнений, входящих в СЛАУ,
/// последний столбей матрицы - значения правых частей уравнений
/// rows - количество строк в исходной матрице
/// result - массив ответов СЛАУ
void SerialGaussMethodCilk(double **matrix, const int rows, double* result)
{
	int k;
	...
	high_resolution_clock::time_point t2 = high_resolution_clock::now();
	duration<double> duration = (t2 - t1);
	printf("Cilk forward elimination duration = %f ticks\n\n", duration.count());
...
int main()
{
	...
	SerialGaussMethodCilk(matrix, MATRIX_SIZE, result);
